...
public class PCma_GuiAutomaticWorkbench extends PCma_ContainerAutomaticWorkbench implements PC_IGresGui, PC_IGresEventListener {
	...
	public void initGui(PC_GresGuiHandler gui) {
		...
		window.addSideTab(PC_GresWindowSideTab.createRedstoneSideTab(automaticWorkbench));
		window.addSideTab(PC_GresWindowSideTab.createIOConfigurationSideTab((PC_ISidedInventory)automaticWorkbench));
		...
		for(int j=0; j<3; j++){
			for(int i=0; i<3; i++){
				inv.setSlot(i, j, invSlots[j*3+i]);
			}
		}
		group.add(inv = new PC_GresInventory(1, 1));
		inv.setSlot(0, 0, invSlots[9]);
		window.add(group);
		window.add(inv = new PC_GresInventory(9, 2));
		for(int j=0; j<2; j++){
			for(int i=0; i<9; i++){
				inv.setSlot(i, j, invSlots[10+j*9+i]);
			}
		}
