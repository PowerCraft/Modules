...
public final class PCco_CraftingToolCrafter {
	...
	private static int testItem(List<ItemStack> get, ItemStack[] is, List<ItemStack> not, int rec, EntityPlayer thePlayer) {
		int i = 0;
	...
	static void onPacket(EntityPlayerMP player, ItemStack itemStack) {
		ItemStack[] pi = PCco_CraftingToolCrafter.getPlayerInventory(player);
		itemStack.stackSize = PCco_CraftingToolCrafter.craft(itemStack, pi, new ArrayList<ItemStack>(), 0, player);
		if(itemStack.stackSize>0){
			...
			}else if(pis.isItemEqual(itemStack)){
				if(pis.stackSize+itemStack.stackSize<=itemStack.getMaxStackSize()){
					pis.stackSize += itemStack.stackSize;
					PCco_CraftingToolCrafter.setPlayerInventory(pi, player);
